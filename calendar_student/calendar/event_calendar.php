...
$stmt1 = $conn1->prepare('SELECT title, description, start_datetime, end_datetime FROM acad_student_events WHERE id = ?');
// Use ID to get info
$stmt1->bind_param('i', $_SESSION['id']);
$stmt1->execute();
$stmt1->bind_result($title, $description, $start_datetime, $end_datetime);
$stmt1->fetch();
$stmt1->close();
//s2
// Database gather through sessions
$stmt2 = $conn2->prepare('SELECT title, course, year, description, start_datetime, end_datetime FROM acad_staff_events WHERE id = ?');
// Use ID to get info
...
//s3
// Database gather through sessions
$stmt3 = $conn3->prepare('SELECT title, course, year, description, start_datetime, end_datetime FROM acad_admin_events WHERE id = ?');
// Use ID to get info
$stmt3->bind_param('i', $_SESSION['id']);
$stmt3->execute();
$stmt3->bind_result($title, $course, $year, $description, $start_datetime, $end_datetime);
$stmt3->fetch();
$stmt3->close();
?>
...
<head>
    ...
    <title>Acadcalendar - Student Calendar</title>
...
<body>
    ...
    <header id="header" class="header fixed-top">
        <div class="container-fluid container-xl d-flex align-items-center justify-content-between">
          ...
          <nav id="navbar" class="navbar">
            <ul>
              <li><a class="nav-link scrollto active" href="../student_profile.php">Profile</a></li>
              <li><a class="nav-link scrollto" href="../student_logout.php">Logout</a></li>
            </ul>
                    ...
                    <div class="modal-body rounded-0">
                        <div class="container-fluid">
                            <dl>
                                ...
                                <dd id="course" class="" data-toggle="tooltip" data-placement="bottom" title="1-BSIT / 2-BSHM / 3-BSBA / 4-BSTM / 5-Announcement " >
                                    <?php
                                       echo "0 - Personal";
                                    ?>
                                </dd>
                                <dt class="text-muted">Year Code</dt>
                                <dd id="year" class="" data-toggle="tooltip" data-placement="bottom" title="1-1st / 2-2nd / 3-3rd / 4-4th / 5-Everyone"> 
                                <?php 
                                        echo "0 - Personal";
                                    ?>
                                </dd>
    ...
    <!-- Display Events -->
    <?php 
    $schedules1 = $conn1->query("SELECT * FROM `acad_student_events`");
    $schedules2 = $conn2->query("SELECT * FROM `acad_staff_events`");
    $schedules3 = $conn3->query("SELECT * FROM `acad_admin_events`");
    $sched_res = [];
    ...
    foreach($schedules3->fetch_all(MYSQLI_ASSOC) as $row){
        $row['sdate'] = date("F d, Y h:i A",strtotime($row['start_datetime']));
        $row['edate'] = date("F d, Y h:i A",strtotime($row['end_datetime']));
        $sched_res[$row['id']] = $row;
    }
    ...
    <script src="http://acadcalendar.42web.io/assets/js/student_calendar.js"></script>
